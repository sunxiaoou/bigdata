...
public class ReplicateSvc implements AdminProtos.AdminService.BlockingInterface {
    private static final Logger LOG = LoggerFactory.getLogger(ReplicateSvc.class);

    private final BlockingQueue<Pair<List<AdminProtos.WALEntry>, CellScanner>> queue;

    public ReplicateSvc(BlockingQueue<Pair<List<AdminProtos.WALEntry>, CellScanner>> queue) {
        this.queue = queue;
    }
