...
def dt_login(request, from_modal=False):
  ...
  if request.method == 'POST':
    ...
    if first_user or not is_first_login_ever:
      ...
      if auth_form.is_valid():
        ...
        elif request.headers.get('Response-type') == 'application/json':
          response = {
            'status': 0,
  ...
  if request.headers.get('Response-type') == 'application/json' and request.method == 'GET':
    response = {'status': -1}
    csrf_token = get_token(request)
    response['status'] = 0
    response['csrfmiddlewaretoken'] = csrf_token
    response['csrftoken'] = request.META["CSRF_COOKIE"]
    return JsonResponse(response)
